...
- **同义词学习** — 同义词词库进入「开始学习」自动走同义词翻卡模式（首页按词库类型智能分发，无需单独选择入口）；正面英文+中文释义、背面英文同义词+中文释义；同义词词库的文字测验自动用英文同义词作为选项
- **单词发音** — 卡片音标旁 🔊 按钮，使用浏览器原生 Web Speech API 朗读（en-US）
- **学习测验** — 翻卡结束后立即测验，4 选 1，错题循环重做直到 100% 通关，可「← 上一题」回退改答案；同义词词库自动用英文同义词作为选项
- **测试模式** — 支持**文字测试**与**听力测试**两种类型，从全词库随机出题，可回退改答案（听力题回退后不自动重播）；同义词词库的文字测试自动用英文同义词作为选项（听力测试维持中文选项）
...
操作步骤：
1. 上传 Excel/CSV → 进入列映射页
2. 切换到「同义词模式」（会出现新选项「英文列 2」）
3. 设置：英文列 = B (文章)、英文列 2 = C (题目)、中文列 = D (中文翻译)
4. 后端会把每行展开为两条互为同义词的词条；中文按第一个 `/` 拆分对应

> 中文若无 `/` 分隔符，将整段归到第一英文列对应词，第二英文列对应词的中文为空（预览页可手工补全）。
